...
pretrained_pipeline_03 = models.get_pretrained_pipeline(
    params.PRETRAINED_PIPELINE_NAME, min_cluster_size=3
)
pretrained_pipeline_06 = models.get_pretrained_pipeline(
    params.PRETRAINED_PIPELINE_NAME, min_cluster_size=6
)
pretrained_pipeline_09 = models.get_pretrained_pipeline(
    params.PRETRAINED_PIPELINE_NAME, min_cluster_size=9
)
pretrained_pipeline_12 = models.get_pretrained_pipeline(
    params.PRETRAINED_PIPELINE_NAME, min_cluster_size=12
)
...
run_timestamp = "MY_DATASET_RERUN"
# run_timestamp = "MY_DATASET_1_VIDEO"
...
for video_file in video_list.VIDEO_FILES:
    ...
    video_durations[video_file.save_name] = (
        len(AudioSegment.from_file(audioFilePath)) / 1000
    )
    ...
    (
        final_output[video_file.save_name]["video_simple"],
        final_output[video_file.save_name]["video_enhanced"],
    ...
    final_output[video_file.save_name]["audio_03"] = (
        audioDiarization.perform_audio_diarization(
            pretrained_pipeline=pretrained_pipeline_03,
            pipeline_name="03",
            videoDuration=video_durations[video_file.save_name],
        )
    )
    final_output[video_file.save_name]["audio_06"] = (
        audioDiarization.perform_audio_diarization(
            pretrained_pipeline=pretrained_pipeline_06,
            pipeline_name="06",
            videoDuration=video_durations[video_file.save_name],
        )
    )
    final_output[video_file.save_name]["audio_09"] = (
        audioDiarization.perform_audio_diarization(
            pretrained_pipeline=pretrained_pipeline_09,
            pipeline_name="09",
            videoDuration=video_durations[video_file.save_name],
        )
    )
    final_output[video_file.save_name]["audio_12"] = (
        audioDiarization.perform_audio_diarization(
...
with open(
    os.path.join(
        params.RUN_OUTPUT_FOLDER, f"final_diarization_output_{run_timestamp}.pckl"
    ),
    "wb",
) as f:
...
for video_file in video_list.VIDEO_FILES:
    ...
    if (
        video_file.save_name not in final_output.keys()
        or len(final_output[video_file.save_name]) < 6
    ):
    ...
    final_output[video_file.save_name]["combined_simple"] = (
        combined_output_simple.get_diarization_output(
            videoDuration=video_durations[video_file.save_name],
    ...
    final_output[video_file.save_name]["combined_enhanced"] = (
        combined_output_enhanced.get_diarization_output(
            videoDuration=video_durations[video_file.save_name],
            video_save_name=video_file.save_name,
...
for video_file in video_list.VIDEO_FILES:
    ...
    if video_file.ground_truth_type == "der":
        _result = evaluate_results.perform_evaluation(
            videoDuration=video_durations[video_file.save_name]
        )
...
if len(stats_df_list) > 0:
    ...
    final_stats_df.to_excel(
        os.path.join(
            params.RUN_OUTPUT_FOLDER, f"temp_final_stats_{run_timestamp}.xlsx"
        ),
        index=False,
    )
...
if len(final_evaluation_results_wder) > 0:
    ...
    results_df["Total"] = (
        results_df["Correct"] + results_df["Confusion"] + results_df["Missed"]
    )
    results_df["Accuracy"] = results_df["Correct"] / results_df["Total"]
    results_df["Error_Rate"] = (
        results_df["Confusion"] + results_df["Missed"]
    ) / results_df["Total"]
    results_df.to_excel(
        os.path.join(
            params.RUN_OUTPUT_FOLDER,
